          ...
          <div class="col-lg-12">          
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Change Password {{ $user->name.' ('.$user->email.')' }}</h3>
              </div>
              <div class="card-body">                
                <form action="{{ url('adminpanel/users/'.$user->id.'/change_password') }}" method="post">
                  @method('patch')
                  @csrf
                  <div class="form-group row">
                    <label class="col-sm-2 offset-sm-1 col-form-label">Password Baru</label>
                    <div class="col-sm-8">
                      <input type="password" class="form-control" name="password" value="" placeholder="Password Baru" required>                      
                      @error('password')
                        <code>
                            {{ $message }}
                        </code>
